  ...
  ipcMain.on('code-add', (event: IpcMainEvent, { tabId, code, cursor }: CodeAddPayload) => {
    try {
      ...
      const runAdd = db.transaction(() => {
        const currentState = db.prepare('SELECT current_history_id FROM tab_states WHERE tab_id = ?').get(tabId) as { current_history_id: number | null } | undefined;

        if (currentState?.current_history_id) {
          const lastHistory = db.prepare('SELECT code FROM code_histories WHERE id = ?').get(currentState.current_history_id) as { code: string } | undefined;

          if (lastHistory && lastHistory.code === code) {
            db.prepare('UPDATE code_histories SET cursor_line = ?, cursor_column = ? WHERE id = ?')
              .run(cursor.lineNumber, cursor.column, currentState.current_history_id);
            return;
          }
        ...
        db.prepare('INSERT OR REPLACE INTO tab_states (tab_id, current_history_id) VALUES (?, ?)').run(tabId, newHistoryId);
      });
  ...
  ipcMain.on('code-undo', (event: IpcMainEvent, tabId: number) => {
    try {
      ...
      const runUndo = db.transaction(() => {
        const currentState = db.prepare('SELECT current_history_id FROM tab_states WHERE tab_id = ?').get(tabId) as { current_history_id: number } | undefined;

        if (!currentState) {
          return;
        }

        const selectSql = 'SELECT id, code, cursor_line, cursor_column FROM code_histories WHERE tab_id = ? AND id < ? ORDER BY id DESC LIMIT 1';
        const previousState = db.prepare(selectSql).get(tabId, currentState.current_history_id) as { id: number; code: string; cursor_line: number; cursor_column: number } | undefined;
  ...
  ipcMain.on('code-redo', (event: IpcMainEvent, tabId: number) => {
    try {
      ...
      const runRedo = db.transaction(() => {
        ...
        const selectSql = 'SELECT id, code, cursor_line, cursor_column FROM code_histories WHERE tab_id = ? AND id > ? ORDER BY id ASC LIMIT 1';
        const nextState = db.prepare(selectSql).get(tabId, currentState.current_history_id) as { id: number; code: string; cursor_line: number; cursor_column: number } | undefined;
